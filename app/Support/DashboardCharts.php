...
class DashboardCharts
{
    ...
    public static function multiSeriesSvg(
        ...
        foreach ($yTicks as $i => $v) {
            ...
            $svg .= "<line x1=\"{$padL}\" x2=\"" . ($width - $padR) . "\" y1=\"{$yPos}\" y2=\"{$yPos}\" stroke=\"#E2E8F0\" stroke-width=\"1\" stroke-dasharray=\"{$dash}\" />";
            $svg .= "<text x=\"" . ($padL - 8) . "\" y=\"" . ($yPos + 3) . "\" text-anchor=\"end\" font-size=\"10\" fill=\"#CBD5E1\" font-family=\"'Geist Mono',ui-monospace,monospace\">{$label}</text>";
        }
        ...
        foreach ($xTicks as $i) {
            ...
            $svg .= "<text x=\"{$xPos}\" y=\"{$yPos}\" text-anchor=\"middle\" font-size=\"10\" fill=\"#CBD5E1\" font-family=\"'Geist Mono',ui-monospace,monospace\">{$lbl}</text>";
        }
    ...
    {
        return "<svg width=\"100%\" viewBox=\"0 0 {$width} {$height}\" style=\"display:block\"><text x=\"" . ($width / 2) . "\" y=\"" . ($height / 2) . "\" text-anchor=\"middle\" fill=\"#CBD5E1\" font-size=\"12\" font-family=\"'Geist',sans-serif\">Sin datos para este período</text></svg>";
    }
}
